        ...
        while self.queue:
            ...
            for u in v.neighbors:
                if (u.x, u.y) not in visited:
                    ...
                    u.rhs = min(u.rhs, v.rhs + cost(u, v) + cost(v,self.graph.goal))
                    u.calculate_key()
                    self.queue.add(u)
...
class DijkstrasSolver(PriorityQueueSolver):
    def compute_path(self):
        visited = set()
        while self.queue:
            v = self.queue.pop(0)
...
class GreedyBestFirstSolver(PriorityQueueSolver):
    def compute_path(self):
        ...
        while self.queue:
            ...
            for u in v.neighbors:
                if (u.x, u.y) not in visited:
                    self.queue.discard(u)
                    u.rhs = min(u.rhs,v.rhs + cost(v,self.graph.goal))
                    u.calculate_key()
                    self.queue.add(u)
